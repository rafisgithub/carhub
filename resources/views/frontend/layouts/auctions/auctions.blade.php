...
<script type="text/javascript">
    ...
    <section class="live--auctions--area auction--result">
        <div class="container">
                ...
                <div class="col-12">
                    <div class="filter--and--category">
                        ...
                        <div class="category--and--sort" data-aos="fade-zoom-in" data-aos-duration="400"
                            ...
                            <!-- car category  -->
                            <select id="GetCategory" class="category--select">
                            ...
                            <div class="sort--by">
                                <p>Sort By</p>
                                <ul>
                                    <li class="active" id="endingSoong">Ending Soon</li>
                                    <li id="NewlyListAuctions">Newly listed</li>
                                    <li>Closest to Me</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="filtered--options mt_30" data-aos="fade-zoom-in" data-aos-duration="400"
                            ...
                            <!-- Car Brand select  -->
                            <select class="brand--select" id="GetcarBrands">
                                <option selected disabled>Select Car Brand</option>

                            </select>
                            <!-- car model select  -->
                            <select class="car-model--select" id="GetcarModels">
                            ...
                            <select class="price-range--select">
                                ...
                                <option value="50up">Above $50,000</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="live--auctions--wrapper">
...
@push('scripts')
    <script>
        $(document).ready(function() {
           $('.category--select , .year--select, .brand--select ,.price-range--select, .car-model--select').on('change', function() {
               var category_id = $(this).val();
               let year = $('.year--select').val();
               let brand = $('.brand--select').val();
                let model = $('.car-model--select').val();
                let price = $('.price-range--select').val();

               $.ajax({
                   url: "{{ route('filter.car.auction') }}",
                   type: "GET",
                   data: {
                   ...
                   success: function(response) {
                        ...
                        $.each(response.data, function(key, value){
                            console.log(value.auction_time.end_time);
                            let img = "{{ asset('') }}" + value.thumbnail;
                            console.log(img);
                            let endTime = new Date(value.auction_time.end_time);
                            let currentTime = new Date();
                            let timeDiff = endTime - currentTime;
                            let days = Math.floor(timeDiff / (1000 * 60 * 60 * 24));
                            console.log(days);
                            let carDetailsRoute = "{{ route('car.details', ':id') }}";
                            carDetailsRoute = carDetailsRoute.replace(':id', value.id);
                                ...
                                $('#allAuctions').append(`
                                            ...
                                            <div class="car--model">
                                            <a href="${carDetailsRoute}">${value.car_transmission.transmission_type}</a>
                                            <p>
                                                ${value.year} | ${value.mileage} miles | ${value.engine}
                                            </p>
                                            <div class="card--footer">
                                                <p>Fort Lauderdale, FL 33315</p>
                                                <a href="${carDetailsRoute}" class="buttonv2 button">Place a Bid</a>
                                            </div>
                                            </div>
                                        </div>
                                        </div>
           ...
           $.ajax({
               ...
               success: function(response) {
                if(response.status){
                    $.each(response.data, function(key, value){
                        // create option element using createElement function
                        var option = document.createElement('option');
                        option.text = value.category;
                        option.value = value.id;

                        $('#GetCategory').niceSelect();
                        $('#GetCategory').append(option);
                        $('#GetCategory').niceSelect('update');
                ...
                success: function(response) {
                     if(response.status) {
                          $.each(response.data, function(key, value){
                            ...
                            $('#GetcarBrands').niceSelect();
                            $('#GetcarBrands').append(option);
                            $('#GetcarBrands').niceSelect('update');
                          });
                     }
                }
              });
                    ...
                    success: function(response) {
                         if(response.status) {
                            $.each(response.data, function(key, value){
                                ...
                                $('#GetcarModels').niceSelect();
                                $('#GetcarModels').append(option);
                                $('#GetcarModels').niceSelect('update');
                            });
                         }
                    }
                });
        ...
        // get all auctions
           ...
           $('#endingSoong').on('click', function() {
            $.ajax({
               url: "{{ route('ending.soon.auctions') }}",
               type: "GET",
               success: function(response) {
                $('#allAuctions').html('');
                if(response.status) {
        ...
        // get newly listed auctions

        $('#NewlyListAuctions').on('click', function() {
            $.ajax({
               url: "{{ route('newly.listed.auctions') }}",
               type: "GET",
               success: function(response) {
                $('#allAuctions').html('');
                if(response.status) {
